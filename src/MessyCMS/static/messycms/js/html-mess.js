...
class HtmlMess {
    ...
    constructor(conf) {
        ...
        this.postprocessEvent = 'load';
        
        if (typeof conf === 'object') {
            Object.assign(this, conf)
            if (conf.ajaxModeContainer) {
                this.initAjaxMode(conf.ajaxModeContainer);
            }
        }
    ...
    waitForSuccess(callback, times, period) {
        const This = this;
    ...
    postprocessAll(event) {
        for (let element of document.querySelectorAll('[data-html-mess]')) {
            if (element.dataset.done) {
                continue;
            }
            element.dataset.done = true;
            
            if (element.dataset.includeUrl) {
                fetch(element.dataset.includeUrl, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                })
            ...
            if (element.dataset.moveTo) {
                const moveTo = document.querySelector(element.dataset.moveTo);
                if (moveTo) {
